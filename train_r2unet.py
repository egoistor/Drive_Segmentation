...
def train_net(net, device, data_path, epochs=600, batch_size=1, lr=3e-4):
    ...
    # criterion = nn.
    best_loss = float("inf")
    coslr = LR.LR_Scheduler(optimizer=optimizer,
                            warmup_epochs=5,
                            warmup_lr=lr * 10,
                            num_epochs=epochs,
                            base_lr=lr,
    ...
    for epoch in range(epochs):
        ...
        for images, labels in train_loader:
            ...
            print('epoch:%d  train loss:%f coslr:%f' %
                  (epoch + 1, loss.item(), coslr.get_lr()))
            if loss < best_loss:
                best_loss = loss
